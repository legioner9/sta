...
CNTL: 
    _e : body fn : _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh
    _t : tst_dir : _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.tst.d/_sd2d.tst.d
...
"
    ...
    if [[ "_e" == "${ARGS[0]}" ]]; then
        _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh
        return 0
    fi

    if [[ "_t" == "${ARGS[0]}" ]]; then
        _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.tst.d/_sd2d.tst.d
        return 0
    fi
    ...
    if ! _abs_path "${PPWD}" init_dir; then
        echo "in fs= file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh , line=${LINENO}, ${FNN}() : : EXEC_FAIL : '_abs_path ${PPWD} init_dir' : ${hint} : return 1" >&2
        return 1
    fi
    ...
    if ! [[ -d "$init_dir" ]]; then
        echo "in fs= file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh , line=${LINENO}, ${FUNCNAME}() : NOT_DIR : 'file://$init_dir' : ${hint} : return 1" >&2
        return 1
    fi
    ...
    _in_fn_1_sd2d() {
        ...
        if ! [[ -d "$1" ]]; then
            echo "in fs=file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh, line=${LINENO}, pwd=${PWD} fn=${FUNCNAME}() : NOT_DIR : '${ARGS[0]}'" >&2
            return 1
        fi
        ...
        for item in $(ls -A "$1"); do
            ...
            if [[ -f $dest_path ]]; then
                ...
                if ! _in_fn_1_sd2d $dest_path; then
                    echo "in fs=file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh, line=${LINENO}, pwd=${PWD} fn=${FUNCNAME}() : EXEC_FAIL : '_in_fn_1_sd2d file://$dest_path' return 1" >&2
                    return 1
                fi

            else
                echo "in fs= file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh , line=${LINENO}, _in_fn_1_sd2d() : NOT_DIR or NOT_FILE : 'file://$dest_path' : ${hint} : return 1" >&2
                return 1
            fi
    ...
    if ! _in_fn_1_sd2d $result_dir; then
        echo "in fs=file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sd2d.sh, line=${LINENO}, pwd=${PWD} fn=${FUNCNAME}() : EXEC_FAIL : '_in_fn_1_sd2d $init_dir' return 1" >&2
        return 1
    fi
